...
BEGIN
  ...
  -- Targeted allocation path: any payment pinned to a specific subscription
  -- by the operator (Collect Payment dialog OR Billing record-payment OR
  -- AddTransaction with subscription chosen) must honour that intent and
  -- not spill via FIFO. Covers both 'subscription_payment' (in-profile bill
  -- collection) and 'manual_payment' (Billing worklist + generic) sources.
  IF NEW.source IN ('subscription_payment','manual_payment')
    ...
    IF v_target_subscriber IS NULL THEN
      RAISE EXCEPTION 'targeted payment: subscription % not found', NEW.subscription_id;
    END IF;
    IF v_target_subscriber <> NEW.subscriber_id THEN
      RAISE EXCEPTION 'targeted payment: subscription does not belong to subscriber';
    END IF;
    IF v_target_service <> COALESCE(NEW.service_type, 'cable') THEN
      RAISE EXCEPTION 'targeted payment: service_type mismatch with target subscription';
    END IF;
  ...
  -- Default FIFO path: oldest unpaid subscription first.
  FOR rec IN
    SELECT s.id,
           s.total_charged,
           COALESCE((
