-- Batch A (per docs/LEGACY_COLUMN_AUDIT_2026-06.md): drop legacy pack-id columns.
-- These columns have no frontend readers and no SQL function reads them for
-- business logic. Authoritative source is `subscriptions.pack_id WHERE status='active'`.

-- 1. Drop FKs + indexes that point at the columns.
...
DROP INDEX IF EXISTS public.idx_subscribers_current_pack_id;
DROP INDEX IF EXISTS public.idx_subscribers_current_internet_pack;

-- 2. Rewrite create_subscription / cancel_subscription so they STOP writing
--    the legacy id columns. Otherwise the next call would fail after the
--    column is dropped. Same logic as the 2026-06-19 RPCs minus the
--    current_pack_id / current_internet_pack_id assignments.
...
-- 3. Finally, drop the columns themselves.
ALTER TABLE public.subscribers
  DROP COLUMN IF EXISTS current_pack_id,
  DROP COLUMN IF EXISTS current_internet_pack_id;