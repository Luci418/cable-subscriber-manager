...
-- =============================================================
-- Phase 6.5 Batch B — Drop legacy current_pack / current_internet_pack
-- cached labels and prune all compatibility writes.
-- =============================================================

-- ---------- 1. is_pack_in_use — canonical, relational only ----------
...
-- ---------- 2. create_subscription — stop writing cached label ----------
...
BEGIN
  ...
  -- Batch B: no longer maintain current_pack / current_internet_pack cache.
  -- The JSONB blob columns remain (retired in Batch C).
  IF p_service_type = 'internet' THEN
    UPDATE public.subscribers
...
-- ---------- 3. cancel_subscription — stop clearing cached label ----------
...
BEGIN
  ...
  -- Batch B: no longer clear current_pack / current_internet_pack.
...
-- ---------- 4. expire_lapsed_subscriptions — strip legacy label writes ----------
...
BEGIN
  ...
  -- Batch B: no longer touch current_pack / current_pack_id (columns removed
  -- in Batch A and Batch B). The JSONB blob columns are the last legacy
  -- projection standing — they retire in Batch C.
  FOR rec IN
...
ALTER TABLE public.subscribers
  DROP COLUMN IF EXISTS current_pack,
  DROP COLUMN IF EXISTS current_internet_pack;
