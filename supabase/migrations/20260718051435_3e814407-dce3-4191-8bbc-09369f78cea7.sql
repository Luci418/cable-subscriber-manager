...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM vault.secrets WHERE name = 'credentials_encryption_key') THEN
    PERFORM vault.create_secret(
      encode(gen_random_bytes(32), 'hex'),
      'credentials_encryption_key',
      'Symmetric key for encrypting subscriber/device credentials (PPPoE, WiFi, ONU passwords).'
    );
  END IF;
END$$;
...
ALTER TABLE public.device_assignment_log
  ADD COLUMN IF NOT EXISTS onu_username text,
  ADD COLUMN IF NOT EXISTS onu_password_encrypted text,
  ADD COLUMN IF NOT EXISTS wifi_ssid text,
  ADD COLUMN IF NOT EXISTS wifi_password_encrypted text,
  ADD COLUMN IF NOT EXISTS vlan_id text;

ALTER TABLE public.subscribers
  ADD COLUMN IF NOT EXISTS assigned_telephone text,
  ADD COLUMN IF NOT EXISTS pppoe_username text,
  ADD COLUMN IF NOT EXISTS pppoe_password_encrypted text;

-- 4. Update assignment-log immutability trigger to allow credential edits ----
...
DECLARE
  ALLOWED_COLS constant text[] := ARRAY[
    ...
    'updated_at',
    -- Credential fields — mutable ONLY while the assignment is open.
    'onu_username', 'onu_password_encrypted',
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Guard: credential fields may only change while the assignment is open.
    IF OLD.closed_at IS NOT NULL AND (
         NEW.onu_username             IS DISTINCT FROM OLD.onu_username
      OR NEW.onu_password_encrypted   IS DISTINCT FROM OLD.onu_password_encrypted
      OR NEW.wifi_ssid                IS DISTINCT FROM OLD.wifi_ssid
      OR NEW.wifi_password_encrypted  IS DISTINCT FROM OLD.wifi_password_encrypted
      OR NEW.vlan_id                  IS DISTINCT FROM OLD.vlan_id
    ) THEN
      RAISE EXCEPTION
        'Credentials on assignment id=% are frozen because the assignment is closed.', OLD.id
        USING ERRCODE = 'check_violation';
    END IF;
...
-- 5. Permission helper (server-side canonical) ------------------------------
CREATE OR REPLACE FUNCTION public.can_view_credentials(_uid uuid DEFAULT auth.uid())
 RETURNS boolean
 LANGUAGE sql
...
AS $$
  SELECT decrypted_secret
   ...
   LIMIT 1;
$$;
REVOKE ALL ON FUNCTION public._credentials_key() FROM PUBLIC, anon, authenticated;

-- 7. RPC: read all credentials for a subscriber -----------------------------
...
DECLARE
  ...
  v_key text;
  v_cable_dev public.stb_inventory;
  v_internet_dev public.stb_inventory;
  v_cable_log public.device_assignment_log;
...
BEGIN
  ...
  SELECT * INTO v_cable_dev FROM public.stb_inventory
   WHERE user_id = v_uid AND subscriber_id = p_subscriber_id
     AND status = 'assigned' AND service_type = 'cable'
  ...
  IF v_cable_dev.id IS NOT NULL THEN
    SELECT * INTO v_cable_log FROM public.device_assignment_log
     WHERE user_id = v_uid AND subscriber_id = p_subscriber_id
       AND device_serial = v_cable_dev.serial_number
  ...
  RETURN jsonb_build_object(
    ...
    'cable', CASE WHEN v_cable_dev.id IS NULL THEN NULL ELSE jsonb_build_object(
      'device_id',       v_cable_dev.id,
      'serial_number',   v_cable_dev.serial_number,
      'mac_address',     v_cable_dev.mac_address,
      'assignment_id',   v_cable_log.id,
      'wifi_ssid',       v_cable_log.wifi_ssid,
      'wifi_password',
        CASE WHEN v_cable_log.wifi_password_encrypted IS NULL THEN NULL
             ELSE convert_from(pgp_sym_decrypt(decode(v_cable_log.wifi_password_encrypted,'base64'), v_key), 'utf8') END
    ) END,
...
-- 8. RPC: save ISP identity (subscribers) -----------------------------------
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.subscribers WHERE id = p_subscriber_id AND user_id = v_uid) THEN
  ...
  ELSIF p_pppoe_password IS NULL OR p_pppoe_password = '' THEN
    SELECT pppoe_password_encrypted INTO v_enc FROM public.subscribers WHERE id = p_subscriber_id;
  ELSE
    v_key := public._credentials_key();
    v_enc := encode(pgp_sym_encrypt(p_pppoe_password, v_key), 'base64');
  END IF;
...
-- 9. RPC: save WiFi credentials (open cable assignment log row) -------------
...
BEGIN
  ...
  SELECT dal.id INTO v_log_id
   ...
   WHERE dal.user_id = v_uid
     ...
     AND dal.service_type = 'cable'
     AND dal.closed_at IS NULL
     AND inv.status = 'assigned'
   ORDER BY dal.created_at DESC LIMIT 1;

  IF v_log_id IS NULL THEN
    RAISE EXCEPTION 'No paired cable device — pair a device to configure WiFi credentials.';
  END IF;
...
-- 10. RPC: save ONU / MAC / VLAN credentials --------------------------------
...
BEGIN
  ...
  ELSIF p_onu_password IS NULL OR p_onu_password = '' THEN
    SELECT onu_password_encrypted INTO v_enc FROM public.device_assignment_log WHERE id = v_log_id;
  ELSE
    v_key := public._credentials_key();
    v_enc := encode(pgp_sym_encrypt(p_onu_password, v_key), 'base64');
  END IF;
...
GRANT EXECUTE ON FUNCTION public.can_view_credentials(uuid) TO authenticated;
