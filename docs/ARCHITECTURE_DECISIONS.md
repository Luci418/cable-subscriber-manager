...
## ADR-011 — Transactions are an append-only ledger; corrections via void + replacement

- **Decision**: `public.transactions` is a fully immutable, append-only ledger from the moment a row is written. Financial fields (`amount`, `type`, `service_type`, `subscriber_id`, `provider_id`, `date`, `reverses_transaction_id`) cannot be edited after insert. Transactions cannot be deleted. Only the `description` field (a human-readable note) remains editable. Corrections happen exclusively through **void** (`void_transaction` RPC: inserts an offsetting reversal row and flips the original to `status = 'voided'`) followed, if appropriate, by a **replacement** transaction posted by the operator. Status transitions are restricted to `posted → voided` via the RPC; all other transitions are rejected. Enforcement is in the database via the `transactions_enforce_immutability` BEFORE UPDATE/DELETE trigger.
- **Context**: With the Tier-0 balance trigger, the ledger is the single source of financial truth. A rewritable source of truth is not a source of truth. The earlier "edit/delete with password `1234`" model destroyed history silently. An interim revision of this ADR allowed a 5-minute grace window for genuine fat-finger fixes; on review (2026-06-08) we concluded the grace window adds operational complexity (timing logic in the UI, "why can't I edit this anymore?" surprises, two delete paths) without earning meaningful safety, because the void workflow already handles fat-finger fixes cleanly and auditably. See `docs/FINANCIAL_LIFECYCLE_REVIEW_2026-06.md`.
- **Alternatives**: (a) status quo mutable ledger; (b) 5-minute grace window with hard delete (the previous revision of this ADR); (c) soft-delete via `deleted_at`; (d) split into separate `payments`/`charges`/`refunds` tables.
- **Reasoning**: Pure immutability is the simplest rule a cashier can remember: "what you save is what's posted; mistakes are voided, not erased." Void/reversal expresses *what happened* in the ledger. Receipt generation is never blocked by a timer because rows are final on insert. Single-table `transactions` keeps the schema small; `status` + `reverses_transaction_id` + `void_reason` are sufficient.
- **Tradeoffs**: Description edits are still lossy (no per-field audit log yet). A typo in `amount` always costs two rows (void + replacement) rather than a silent in-place fix; this is the explicit cost of the audit trail. Operators must be trained that "Void" is the only way to undo.
- **Revisit when**: (a) compliance (GST audit, statutory invoice register) requires per-field history of metadata edits → introduce `transaction_audit_log`; (b) staff expansion demands reason-code taxonomy → promote `void_reason` to an enum; (c) the operator requests month-end period locks → add a `closed_periods` table that gates the void RPC.
