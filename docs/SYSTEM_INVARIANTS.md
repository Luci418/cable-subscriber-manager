...
canonical invariant set: INV-01 … INV-52** (Part Twelve = INV-01…INV-38,
"Additional Invariants" = INV-39…INV-52). This document is the *engineering*
...
Status: **shipped** (2026-08). The engine is live end-to-end; the canonical
design doc is `PROVIDER_SYNC_IMPLEMENTATION_PLAN.md` and the decisions are
ADR-013 … ADR-024. Coverage below is real: `test/db/12_provider_sync_schema.sql`
(17 assertions), `test/db/13_extend_subscription_device.sql` (5 assertions),
and the Vitest suites `diffEngine.test.ts`, `resolution.test.ts`,
`reviewModel.test.ts`, `syncPolicy.test.ts`, `runReport.test.ts`.

| Invariant | Why it exists | Enforced where | Tested? | Docs |
|---|---|---|---|---|
| INV-46 — Provider reports are evidence, never the ledger. Only a **committed** `provider_import_run` may create a transaction; on disagreement the SMS ledger stands until an operator resolves it | The provider portal is not an accounting system; silent overwrite of financial state is unrecoverable | `commit_provider_import` RPC as sole sync write path; `can_sync_provider` role gate | 🟢 `12_provider_sync_schema.sql` (role gate, `provider_sync` enum value, committed-run immutability) | ADR-014, PROVIDER_SYNC_IMPLEMENTATION_PLAN.md |
| INV-47 — Provider sync never edits, deletes, or rewrites existing transactions; corrections are explicit operator actions (adjustment, reversal, reconciliation) | Preserves the append-only ledger under an automated writer | `transactions_enforce_immutability` trigger + insert-only commit RPC | 🟢 `01_transactions_immutable.sql` (trigger); 🟡 commit path itself is manual regression | ADR-011, ADR-015 |
| INV-48 — Idempotency: diff only against the latest **committed** run for `(provider_id, report_type)`; cancelled reviews leave no baseline | Re-uploading a file must never double-charge, and cancelling must never mark events as already-synced | Baseline query `WHERE status='committed' ORDER BY imported_at DESC LIMIT 1`; committed runs immutable | 🟢 `12_provider_sync_schema.sql` (draft editable, committed run rejected) + `diffEngine.test.ts` | ADR-017 |
| INV-49 — Sync never changes subscriber identity fields (name, address, mobile, GST, notes, billing prefs) unless explicitly enabled in `sync_policy` | The SMS owns customer identity; provider data is operational | Sync-policy filter before any proposed write; defaults deny | 🟢 `syncPolicy.test.ts` ("denies identity writes by default") + `resolution.test.ts` ("never proposes an identity write") + `12_provider_sync_schema.sql` (DB default) | ADR-018 |
| INV-50 — `sync_policy` is read only via `getSyncPolicy(provider)`, merging stored JSON over current defaults; a missing key takes its documented default | A future ninth flag must not silently disable itself for every existing provider row | `src/lib/providers/syncPolicy.ts`; no direct `sync_policy.<key>` access | 🟢 `syncPolicy.test.ts` (8 tests: absent key → default, malformed shapes, single-flag reads) | ADR-019 |
| Absence is never termination — only an explicit non-`ACTIVE` `service_status` is evidence | A row missing from an export usually means an export quirk, not a disconnection | Diff engine; missing rows only age `last_seen_in_snapshot_at` | 🟢 `diffEngine.test.ts` | ADR-021 |
| Unknown provider status values are stored verbatim, never bucketed | A guessed mapping can turn an unknown status into a false disconnection | Parser stores the raw string; `is_active` derived as `raw === 'ACTIVE'` only | 🟢 `diffEngine.test.ts` / `resolution.test.ts` (anomaly bucketing, no normalisation) | ADR-021 |
| INV-51 — A committed provider import is a historical record: `provider_plan_key`, `pack_id`, `pack_name`, `pack_price`, `provider_cost` and `parser_version` are frozen onto the run at commit | Renaming or repricing a pack a year later must not rewrite what an old import meant | `commit_provider_import` snapshots pack fields into `results`; `parser_version` frozen at parse; committed runs immutable | 🟢 `runReport.test.ts` ("describes a renewal from the frozen pack, not the report plan") + `12_provider_sync_schema.sql` (run immutability) | ADR-023 |
| INV-52 — A manually entered provider account number is a first-class deterministic identity key; a number claimed by two customers matches nobody | An operator-typed link must be as authoritative as a sync-written one, and an ambiguous key must never pick a winner | `save_provider_account` RPC (role-gated, refuses collisions) + unique index; `loadReviewContext` unions the legacy column with `subscriber_provider_state` and drops ambiguous keys | 🟢 `12_provider_sync_schema.sql` (unique-index collision) + `resolution.test.ts` (account-number match, ambiguity → no match) | ADR-022 |
| Mobile is never a match at all — matching is VC Id → STB serial → account number, and nothing else | Households share numbers; a suggested mobile match was still a footgun during review | Resolution layer (`resolution.ts`) — no mobile lookup exists | 🟢 `resolution.test.ts` (match order + "never matches on mobile") | BUSINESS_RULES §1.1, ADR-022 |
| A vc_id / serial_number disagreement is a conflict, never a silent pick | Two identifiers pointing at two customers means we do not know who this is | Resolution layer — row goes to `needs_review` as a conflict | 🟢 `resolution.test.ts` | ADR-022 |
| A previously failed row is never diffed away as `no_change` | A failed key must resurface on the next import, not vanish because the file is unchanged (baseline-corruption guard) | Diff/resolution carry-forward of `failed_keys` from the last committed run | 🟢 `resolution.test.ts` ("failed rows never diff away") | PROVIDER_SYNC_IMPLEMENTATION_PLAN.md |
| A newly linked customer with an existing active subscription is never charged again | Identity linkage itself triggers a write; without the guard a linked customer would be double-charged | `deriveReview` consumes `subscribersWithActiveSubscription` from `loadResolutionContext` | 🟢 `reviewModel.test.ts` + `resolution.test.ts` ("does NOT trigger when the customer already has an active subscription") | ADR-024 |
| Renewal extends in place; an unresolved device fails the row instead of extending an arbitrary subscription | Multi-STB households would otherwise renew the wrong connection | `extend_subscription` device targeting; `commit_provider_import` pushes the row to `failed_keys` | 🟢 `13_extend_subscription_device.sql` (5 assertions) | ADR-024 |
| Only owners and office admins may run or commit an import | Sync writes money rows | `can_sync_provider()` SECURITY DEFINER gate | 🟢 `12_provider_sync_schema.sql` (all four roles + no-role) | PERMISSION_MATRIX.md |
| Charge amount is always the local catalog price, never `dpo_total_price` | The provider's DPO figure is informational, not a sell price | Review screen + commit RPC | 🟢 `resolution.test.ts` / `reviewModel.test.ts` (charge plan from local pack price) | ADR-023 |
